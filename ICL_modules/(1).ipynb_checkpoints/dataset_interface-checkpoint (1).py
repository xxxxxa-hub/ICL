...
class DatasetSplitter:
    ...
    def get_ground_truth_label_index(self, index, test_set = True) -> int:
        if index < 0 or index >= len(self.test):
            raise ValueError("Index out of range.")
        if test_set:
            return self.test.find_index_from_label(self.get_default_ground_truth_label(index, test_set=True))
        else:
            return self.demonstration.find_index_from_label(self.get_default_ground_truth_label(index, test_set = False))
